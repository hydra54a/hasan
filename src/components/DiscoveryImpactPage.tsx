...
function ScoreRow({ area, onChange, onRemove }: { area: Area; onChange: (key: 'now' | 'after', value: number) => void; onRemove: () => void }) {
  ...
  return <div className="discovery-score-row">
    ...
    <p>{area.quote}</p>
    <label><span>Now</span><input type="range" min="1" max="10" value={area.now} onChange={(event) => onChange('now', Number(event.target.value))} /></label>
    <label><span>After</span><input type="range" min="1" max="10" value={area.after} onChange={(event) => onChange('after', Number(event.target.value))} /></label>
  </div>
}
...
export function DiscoveryImpactPage({ inputs, updateInput }: { inputs: RoiInputs; updateInput: <K extends keyof RoiInputs>(key: K, value: RoiInputs[K]) => void }) {
  ...
  return <>
    ...
    <div className="discovery-page"><div className="discovery-intro">This model shows the <strong>positive impact</strong> an individual rep gains from using the Discovery Framework. Set where the rep is today and where they land after the strategy, then adjust the projection levers to keep the estimate conservative.</div>
      <div className="discovery-grid"><div>
        <section className="discovery-card"><h2>The Rep's Current Numbers</h2><p className="discovery-sub">Everything below builds on these three figures.</p>
        ...
        {(['rep', 'prospect'] as Side[]).map((side) => <section className="discovery-card" key={side}><h2 className={side}>{side === 'rep' ? 'The 5 Sales Representative Improvements' : 'The 5 Prospect Decision Conditions'}</h2><p className="discovery-sub">{side === 'rep' ? 'How the rep shows up on the call.' : 'How the prospect feels and decides.'}</p>{areas.filter((area) => area.side === side).map((area) => <ScoreRow key={area.id} area={area} onChange={(key, value) => updateArea(area.id, key, value)} onRemove={() => setAreas((current) => current.filter((item) => item.id !== area.id))} />)}<button type="button" className="discovery-add" onClick={() => addArea(side)}>+ Add a {side}-side area</button></section>)}
      </div><aside className="discovery-results">
        <section className="discovery-card"><h2>Projection Levers</h2><p className="discovery-sub">Tune how score lifts translate into a close-rate gain.</p><label className="discovery-lever">Impact sensitivity <strong>{sensitivity}%</strong><input type="range" min="5" max="40" value={sensitivity} onChange={(event) => setSensitivity(Number(event.target.value))} /></label><label className="discovery-lever">Conservative factor <strong>{conservative}%</strong><input type="range" min="25" max="100" step="5" value={conservative} onChange={(event) => setConservative(Number(event.target.value))} /></label></section>
